...
export default function InvoicesPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setIsLoading(true)
            const [foldersRes, invoicesRes, clientsRes] = await Promise.all([
                supabase.from('document_folders').select('*').eq('user_id', userId).eq('type', 'invoice').order('created_at', { ascending: false }),
    ...
    const getItemCount = (folderId: string) => invoices.filter(i => i.folder_id === folderId).length
    ...
    const displayedInvoices = activeFolderId === 'all'
        ? invoices
        : activeFolderId
            ? invoices.filter(i => i.folder_id === activeFolderId)
            : []

    const activeFolderName = activeFolderId === 'all' ? 'All Invoices' : folders.find(f => f.id === activeFolderId)?.name
            ...
            <div className="space-y-6">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight text-foreground">
                        {activeFolderId ? activeFolderName : 'Invoice Folders'}
                    </h1>
                    <p className="text-muted-foreground mt-1">
                        {activeFolderId
                            ? 'Manage your generated documents within this folder.'
                            : 'Organize your invoices seamlessly.'}
                    </p>
                </div>
